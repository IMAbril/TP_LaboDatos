...
def profundidad_arbol():
    ...
    for depth in depths:
        ...
        print(f"Profundidad: {depth}, Exactitud Media: {mean_score}")
    ...
    for modelo in depths:
        ...
        resultados_modelos.append({
            'modelo': arbol,
            'exactitud': accuracy_test,
            'informe_clasificacion': classification_rep_test,
            'matriz_confusion': conf_mat_test
        })
    
    # Buscamos el modelo con la mejor exactitud en el conjunto de test
    mejor_modelo = max(resultados_modelos, key=lambda x: x['exactitud'])
    
    # Imprimimos las métricas del mejor modelo para saber todos sus datos
    print(f"Exactitud en conjunto de test: {mejor_modelo['exactitud']}")
...
def grafico_Exactitud_promedio():    
    # Graficamos la exactitud promedio en validación cruzada para cada profundidad
    plt.figure(figsize=(8, 5))
    plt.plot(depths, mean_scores, marker='o', color='green')
    plt.title('Exactitud Media en Validación Cruzada vs. Profundidad del Árbol')
